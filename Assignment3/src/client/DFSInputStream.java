 ...
 *
 * <p>
 * Read calls on a <code>DFSInputStream</code> are directed to a storage server hosting the given file. Each read call
 * corresponds to one network request. If this behavior is not desirable, the <code>DFSInputStream</code> should be
 * wrapped in a <code>BufferedInputStream</code> or other class providing buffered input.
 *
 * <p>
...
public class DFSInputStream extends InputStream {
     ...
     *
     * @param naming_server Stub for the naming server hosting metadata for the file.
     * @param file          Path to the file.
     * @throws FileNotFoundException If the file is not listed by the given naming server, or if the path refers to a
     *                               directory.
     * @throws IOException           If either the naming server or the storage server hosting the file cannot be
     *                               contacted to retrieve file metadata.
     */
     ...
     *
     * @param hostname Address of the naming server hosting metadata for the file.
     * @param file     Path to the file.
     * @throws FileNotFoundException If the file is not listed by the given naming server, or if the path refers to a
     *                               directory.
     * @throws IOException           If either the naming server or the storage server hosting the file cannot be
     ...
     *
     * <p>
     * The stream is marked as closed. Further attempts to use the input stream will result in
     ...
     *
     * <p>
     * The read is performed in a single request to the storage server. If the operation succeeds, the number of bytes
     * read will be either
     ...
     * @param buffer_offset Offset into the buffer at which the bytes are to be written.
     * @param read_length   The maximum number of bytes to read.
     * @return The number of bytes successfully read, or <code>-1</code> if the end of file is reached before any bytes
     * are read. The number of bytes successfully read may be zero if <code>read_length</code> is zero.
     * @throws IOException               If the stream is closed, if the storage server cannot be contacted, or if a
     *                                   read error occurs on the storage server.
     ...
     *                                   or if <code>buffer_offset + read_length</code> exceeds the length of the given
     *                                   buffer.
     */
    @Override
     ...
     *
     * @return The value of the byte read, as an integer between <code>0</code> and <code>255</code>, or <code>-1</code>
     * if the end of file has been reached.
     * @throws IOException If the stream is closed, if the storage server cannot be contacted, or if a read error occurs
     ...
     *
     * <p>
     * The stream offset is advanced by either the given number of bytes, or by the number of bytes remaining in the
     * file, whichever is less.
     *
     * @param count Number of bytes by which the stream offset should be advanced.
     ...
     *
     * <p>
     * The number returned is exact and not an estimate. However, if the number exceeds <code>Integer.MAX_VALUE</code>,
     * it is clamped to
