 ...
 *
 * <p>
 * Temporary directories are created under the directory given by the system property <code>java.io.tmpdir</code>. Each
 * has the name
...
public class TemporaryDirectory {
    ...
    /**
     * Flag used to indicate that the directory has already been removed. This is used to allow multiple calls to
     * <code>remove</code>. Calls after the first one should not remove the directory - because the directory may have
     * been since re-created for another purpose.
     */
     ...
     *
     * @throws FileNotFoundException If a directory cannot be created. This can occur due to permissions problems, or
     *                               due to the exhaustion of temporary directory names.
     */
     ...
     *
     * <p>
     * This method may be called multiple times. If the directory is successfully removed, subsequent calls will do
     * nothing. This ensures that subsequent calls to <code>remove</code> will not delete a new temporary directory
     ...
     *
     * @param path Path to the file, represented as an array of path components. The subdirectory in which the file is
     *             located is created if it does not exist.
     * @return A <code>File</code> object representing the new file.
     * @throws IllegalArgumentException If <code>path</code> represents the temporary directory itself.
     ...
     *
     * <p>
     * This class implements a task which will be run upon JVM shutdown. It contains a weak reference to the temporary
     * directory object. This permits the temporary directory to be cleaned by the garbage collector as if the cleanup
     * task did not exist. If the temporary directory object survives until JVM shutown, this task ensures that the
    ...
    private static class CleanupShutdownHook implements Runnable {
         ...
         *
         * @param directory The temporary directory to be removed on JVM shutdown, if not previously removed by a call
         *                  to
