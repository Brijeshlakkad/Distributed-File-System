 ...
 *
 * <p>
 * test starts a naming server and a test storage server. Items checked are:
 * <ul>
...
public class CreationTest extends NamingTest {
    ...
    /**
     * Indicates that the file <code>expect_file</code> has been created on the storage server by command from the
     * naming server.
     */
    ...
    /**
     * Tests that valid requests to create a file cause a file to be created on the naming server and on the storage
     * server.
     *
     * @param file The path to the file to be created.
     * @throws TestFailed If the file is not created on either the naming or the storage server.
     */
    ...
    /**
     * Checks that the <code>createDirectory</code> method rejects invalid arguments.
     *
    ...
    /**
     * Checks that the <code>createFile</code> method rejects invalid arguments.
     *
     * @throws TestFailed If the test fails.
     */
     ...
     *
     * <p>
     * This storage server waits for calls to <code>create</code> on its command interface. If an expected call is
     * received, it wakes any thread that may be blocked in <code>testFileCreation</code>. If no such call is expected,
     * or the call is received with the wrong argument, the server fails the test.
     */
