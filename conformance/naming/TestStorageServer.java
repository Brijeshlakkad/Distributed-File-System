 ...
 *
 * <p>
 * Instances of this class and its subclasses are connected to the naming server under test. Methods of this class can
 * be overridden to monitor the behavior of the naming server. The default implementations of the methods throw
 * <code>UnsupportedOperationException</code>, indicating that a call to any of the methods in the client or command
 * interfaces is not expected during a test.
 */
class TestStorageServer implements Storage, Command {
    ...
    /**
     * Indicates that the skeletons have been started. This is used to prevent multiple starts of the skeletons by
     ...
     * @throws UnknownHostException If a stub cannot be created from either of the skeletons.
     */
     ...
     *
     * <p>
     * The storage server connects to the given naming server and attempts to register. It provides the naming server
     * with the given list of files. If <code>expect_files</code> is not <code>null</code>, the storage server checks
     * that the naming server then asks it to delete that set of files.
     *
     * @param naming_server The naming server with which the storage server is to register.
     * @param offer_files   The list of files which is to be registered with the naming server.
     * @param expect_files  An optional list of files which the storage server expects the naming server to ask it to
     *                      delete. This parameter can be <code>null</code> if the storage server should not check which
     *                      files the naming server replies with.
     * @return The storage server client interface stub.
     * @throws RMIException         If the storage server skeletons cannot be started, or if the storage server is
     *                              unable to register due to a network error.
     * @throws UnknownHostException If one of the storage server stubs cannot be created.
     * @throws TestFailed           If <code>expect_files</code> is not <code>null</code> and the naming server does not
     *                              reply with the correct set of duplicate files.
     */
