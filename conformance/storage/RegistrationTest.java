 ...
 *
 * <p>
 * This test starts a special testing naming server, creates and populates temporary directory, and starts the storage
 * server in that directory. It then commands the storage server to connect to the naming server, checking that the
 * storage server transmits the correct directory listing and removes files from the directory as directed by the naming
 * server. It also checks that the storage server has correctly pruned empty directories.
 */
