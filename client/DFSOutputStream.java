 ...
 *
 * <p>
 * Write calls on a <code>DFSOutputStream</code> are directed to a storage server hosting the given file. Each call
 * corresponds to one network request. If this is not desirable, the <code>DFSOutputStream</code> should be wrapped in a
 * <code>BufferedOutputStream</code> object.
 *
 * <p>
 * Creating a <code>DFSOutputStream</code> for a file does not cause the file to be created or truncated. The file must
 * exist, and the existing file data is left in place. Writes to the stream cause file data to be overwritten, starting
...
public class DFSOutputStream extends OutputStream {
     ...
     *
     * <p>
     * The stream is marked as closed. Further attempts to use the output stream will result in
     ...
     * @param buffer_offset Offset into the buffer from which bytes are to be written.
     ...
     *                                   write error occurs on the storage server.
     ...
     *                                   or if <code>buffer_offset + write_length</code> exceeds the length of the given
     *                                   buffer.
     */
    @Override
     ...
     * @throws IOException If the stream is closed, if the storage server cannot be contacted, or if a write error
     *                     occurs on the storage server.
     */
    @Override
