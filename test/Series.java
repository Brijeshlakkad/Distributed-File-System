 ...
 *
 * <p>
 * A test series is a group of related tests. A <code>Series</code> object provides the <code>run</code> method, which
 * attempts to run all the tests in the series, and generates a test report.
 *
 * <p>
 * Tests are generally run in the order in which they are provided. However, some tests may explicitly declare other
 * tests in the series as prerequisites. Tests that declare prerequisites are run only after the prerequisites have
 * completed successfully. Note that a test with prerequisites is not run in three cases: the prerequisite has failed,
 * the prerequisite has itself not run, or the prerequisite is not included in the test series. The requirement that all
 * prerequisites be explicitly included in the the test series is deliberate. The test series object notionally captures
 * all the tests that will be run as part of the series.
 *
 * <p>
 ...
 *
 * <p>
 * The tests constituting a test series are provided as a list of classes. A test object is only instantiated
 * immediately before the test is run. This ensures that it is possible to construct a test series even if the
 * constructor of a test object would raise an exception - because test objects need not be constructed to construct a
 * test series, only to run the test.
 *
 * <p>
 * When running a test, the test series object creates a new thread. The thread constructs the test, and calls the
 * test's <code>initialize</code> and
 * <code>perform</code> methods. The test series object also starts a parallel
 * timeout thread. The main test thread, any other threads started by it, and the timeout thread then race to call
 * <code>success</code> or
 * <code>failure</code>. The first call to either method determines the outcome
 * of the test.
 *
 * <p>
 * Once either method is called, the test series object creates a thread that calls <code>clean</code> on the test
 * object, and a parallel cleanup timeout thread. The threads again race - this time to call
 * <code>cleanupSuccess</code> or <code>cleanupFailure</code>. In the event of
 * a cleanup failure, the test series has suffered a fatal error, and no more tests will be run, even if there are other
 * tests for which all prerequisites have completed successfully. This is to prevent failures in those tests caused by
 * unreleased system resources. A fatal error typically causes the process running the test series to stop the Java
 * runtime, forcing the release of the system resources.
 */
public class Series implements Serializable {
     ...
     * @throws NullPointerException If <code>tests</code> is <code>null</code>, or if any of the elements of
     ...
     *
     * <p>
     * Tests are run in the order they were given to the <code>Series</code> constructor, subject to constraints imposed
     * by test prerequisites.
     *
     * @param timeout Test timeout interval, in seconds. If a test fails to complete in this amount of time, it is
     *                considered to have timed out. Likewise, if a test fails to clean up in this amount of time, the
     *                cleanup process is considered to have failed. Note that a test can take twice this period to run:
     *                one period to time out the main test, and one more period to time out the cleanup process.
     * @param stream  Output stream to receive testing progress messages. Each message takes the form of the test class
     *                notice message, or a default replacement if the test class does not have a notice message. When
     *                the test is complete, either
     *                <code>ok</code> or <code>failed</code> is printed to the
     *                stream. If this argument is <code>null</code>, no progress printing is done.
     * @return A <code>SeriesReport</code> object listing the tests that have succeeded, tests that have failed, and
     * tests that were not run due to prerequisites that were missing or did not complete successfully.
     */
     ...
     *
     * <p>
     * This class contains fields that indicate whether the test object has been constructed, whether the test has been
     * stopped, whether cleanup has been completed, and the causes of any failures that occurred.
     *
     * <p>
     * These fields cannot be placed in <code>Test</code> because test objects come into existence only after the test
     * is started. In fact, it is possible for a test to stop before the test object has even been created - for
     * example, in case timeout occurs while the constructor is still running.
     *
     * <p>
     ...
     * that all the threads started by one test have terminated before another test starts running. Because of this, it
     * is possible that one test's delayed thread will call <code>Test.success</code> or
     * <code>Test.failure</code> after the next test has started. In that case,
     * if the fields affected by that call were in <code>Series</code> and shared between all tests, the delayed thread
     * from the old test would terminate the next test.
     */
    static class TestState {
        ...
        Test test = null;
        /**
         * Flag set if the <code>initialize</code> method has completed. The thread calling <code>clean</code> will not
        ...
        /**
         * Current task message. Regular (not cleanup) failures are wrapped in a <code>FailedDuringTask</code> object if
         * they occur when this is not <code>null</code>.
         */
        String task = null;
         ...
         * notified. If the method has alreay been called, calling it again has no effect.
         *
         * <p>
         * The test termination cause is set from the parameter. If the cause is <code>null</code>, the test is
         * considered to have succeeded. Otherwise, the test failed. <code>Test.success</code> calls this method with
         * <code>null</code> as argument, and
         * <code>Test.failure</code> calls this method and passes its argument.
         *
         * <p>
         * If the test is being stopped due to a failure, and there is a task message set when the failure occurrs, then
         * the failure is wrapped in a <code>FailedDuringTask</code> object which stores the task message.
         *
         ...
         *
         * <p>
         * This method may only be called after the test has stopped. Calling this method earlier will cause the test to
         * be terminated with a fatal error.
         *
         * <p>
         * If the method has not yet been successfully called, the cleanup process is stopped. The
         * <code>cleanup_stopped</code> flag is set and any waiting threads are notified. If the method has already been
     ...
     * given test class, if such a field exists. If the field does not exist, or does not meet these requirements, the
     * empty array is used.
     *
     * <p>
     * After obtaining the list of prerequisites in this manner, the method then proceeds to check whether each one is
     * in the list of tests that have succeeded. If so, the method returns <code>true</code>. Otherwise, the method
     ...
     *
     * <p>
     * The field must be public, static, and final. It must have a type that is assignable to the given type, its name
     * must match the given name, and it must not be <code>null</code>. If the given class has no such field, the method
     * returns the given default value.
     *
     * @param c             Class from which the field is to be retrieved.
     * @param field_type    Field type. The actual field type may be any type that is assignable to this t ype.
     * @param name          Field name.
     * @param default_value Default value to be used if the actual field value cannot be retrieved.
     * @return The value of the field, or the default value if a field matching the requirements cannot be found in the
     * class.
     */
